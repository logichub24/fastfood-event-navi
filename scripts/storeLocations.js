...
// * KFC/서브웨이는 이 데이터셋에 잡히지 않고 자체 매장찾기도 세션/CSRF나 SPA라 직접 크롤이 어려움 ->
//   카카오 로컬 API 키워드 검색(kakao-stores.js)으로 수집한다. (KFC 186건, 서브웨이 411건)
// * 노브랜드버거(주문 사이트 SPA)/프랭크버거(별도 API 없음)는 이 데이터셋 매칭 + 카카오 보완으로 대체.
